...
namespace cf01.Forms
{
    public partial class frmStoreIssue : Form
    {
        ...
        private void frmStoreIssue_Load(object sender, EventArgs e)
        {
            ...
            Font a = new Font("GB2312", 9);//GB2312为字体名称，1为字体大小dataGridView1.Font = a;
            dgvDetails.Font = a;
        ...
        private void loadFlRec(int sl)
        {
            ...
            rows = dtWipRec.Select("fl_qty>0"); // 从dtWipRec表中查询符合条件的记录；
        ...
        private void findStore()
        {
            ...
            {
                total_qty = total_qty + Convert.ToInt32(dtStore.Rows[i]["qty"]);
                total_weg = total_weg + Convert.ToSingle(dtStore.Rows[i]["sec_qty"]);//Math.Round(
            }
