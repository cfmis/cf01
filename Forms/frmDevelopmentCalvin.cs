...
{
    public partial class frmDevelopmentCalvin : Form
    {
        ...
        private void frmDevelopmentCalvin_Load(object sender, EventArgs e)
        {
            //Load_Date();
            const string sql = @"SELECT * From development_calvin with(nolock) where 1=0 ";
        ...
        private void AddNew()
        {
            ...
            txtform_date1.Text = DateTime.Now.Date.ToString("yyyy/MM/dd").Substring(0, 10);
            txtserial_no.Text = clsTommyTest.GetSeqNo("development_calvin","serial_no");
        ...
        {
            txtserial_no.Focus();
            if (txtserial_no.Text == "" && txtform_date1.Text == "")
            {
            ...
            @"INSERT INTO development_calvin(serial_no,season,form_date1,form_date2,trim_code,requests_by,delivery,pvh_jv_ref,supplier_name,material,size,country,color,artwork_limit,
            ...
            quality_approval,size_approved_color,submit1,submit2,submit3,urgent,ck_mens,ck_womens,ck_df,ck_formal,ck_kids,ck_accessories,ck_jeans,ck_swim,ck_underwear,mk,ck_sportswear,
            mo_id1,mo_id2,mo_id3,color1,color2,color3,size1,size2,size3,create_by,create_date) 
            VALUES(@serial_no,@season,@form_date1,@form_date2,@trim_code,@requests_by,@delivery,@pvh_jv_ref,@supplier_name,@material,@size,@country,@color,@artwork_limit,
            @tiered_qty1,@price_size1,@m_id1,@price1,@p_unit1,@tiered_qty2,@price_size2,@m_id2,@price2,@p_unit2,@tiered_qty3,@price_size3,@m_id3,@price3,@p_unit3,@tiered_qty4,@price_size4,@m_id4,@price4,@p_unit4,@tiered_qty5,@price_size5,@m_id5,@price5,@p_unit5,
            @moq_usa,@surcharge_usa,@moq_eur,@surcharge_eur,@leadtime_sample,@leadtime_bulk,@trim_must_tommy,@oekotex,@suggested_care,@machine_washable,@drycleanable,@dryclean_only,@do_not_dryclean,
            @use_for_swimwear,@use_for_childrenswear,@cannot_pass_needle,@quality_callouts,@used_on_sms_eur,@latest_submit_ref,@best_vendor_do,@bulk_reference,@color_approved_size,
            @quality_approval,@size_approved_color,@submit1,@submit2,@submit3,@urgent,@ck_mens,@ck_womens,@ck_df,@ck_formal,@ck_kids,@ck_accessories,@ck_jeans,@ck_swim,@ck_underwear,@mk,@ck_sportswear,
            @mo_id1,@mo_id2,@mo_id3,@color1,@color2,@color3,@size1,@size2,@size3,@user_id,getdate())";

            const string sql_update =
            @"Update development_calvin 
            SET season=@season,form_date1=@form_date1,form_date2=@form_date2,trim_code=@trim_code,requests_by=@requests_by,delivery=@delivery,pvh_jv_ref=@pvh_jv_ref,supplier_name=@supplier_name,material=@material,size=@size,country=@country,color=@color,artwork_limit=@artwork_limit,
            ...
            moq_usa=@moq_usa,surcharge_usa=@surcharge_usa,moq_eur=@moq_eur,surcharge_eur=@surcharge_eur,leadtime_sample=@leadtime_sample,leadtime_bulk=@leadtime_bulk,trim_must_tommy=@trim_must_tommy,
            oekotex=@oekotex,suggested_care=@suggested_care,machine_washable=@machine_washable,drycleanable=@drycleanable,dryclean_only=@dryclean_only,do_not_dryclean=@do_not_dryclean,
            ...
            quality_approval=@quality_approval,size_approved_color=@size_approved_color,submit1=@submit1,submit2=@submit2,submit3=@submit3,urgent=@urgent,
            ck_mens=@ck_mens,ck_womens=@ck_womens,ck_df=@ck_df,ck_formal=@ck_formal,ck_kids=@ck_kids,ck_accessories=@ck_accessories,ck_jeans=@ck_jeans,ck_swim=@ck_swim,ck_underwear=@ck_underwear,mk=@mk,ck_sportswear=@ck_sportswear,
            mo_id1=@mo_id1,mo_id2=@mo_id2,mo_id3=@mo_id3,color1=@color1,color2=@color2,color3=@color3,size1=@size1,size2=@size2,size3=@size3,update_by=@user_id,update_date=getdate()
            ...
            using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
            {
                try
                {
                    ...
                    if (mState == "NEW")
                    {
                        ...
                        strSerial_no = clsTommyTest.GetSeqNo("development_calvin", "serial_no");
                    ...
                    myCommand.Parameters.AddWithValue("@form_date1", clsApp.Return_String_Date(txtform_date1.Text));
                    myCommand.Parameters.AddWithValue("@form_date2", txtform_date2.Text);
                    ...
                    myCommand.Parameters.AddWithValue("@minimum_sample", txtminimum_sample.Text);
                    myCommand.Parameters.AddWithValue("@minimum_bulk", txtminimum_bulk.Text);
                    //-----------------------------------------------------------------------------------------                    
                    ...
                    myCommand.Parameters.AddWithValue("@ck_mens", chkck_mens.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_womens", chkck_womens.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_df", chkck_df.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_formal", chkck_formal.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_kids", chkck_kids.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_accessories", chkck_accessories.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_jeans", chkck_jeans.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_swim", chkck_swim.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_underwear", chkck_underwear.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@mk", chkmk.Checked ? true : false);
                    myCommand.Parameters.AddWithValue("@ck_sportswear", chkck_sportswear.Checked ? true : false);   
        ...
        private void Delete()
        {
            ...
            {
                const string sql_del = "DELETE FROM dbo.development_calvin WHERE serial_no=@serial_no";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        private void frmDevelopmentCalvin_FormClosed(object sender, FormClosedEventArgs e)
        {
           dtDetail.Dispose();
           dtReSet.Dispose();
           dtMO.Dispose();
        ...
        {
            dtDetail.Clear();
            string sql = @"SELECT * From development_calvin with(nolock) Where 1=1 ";
            if (txtId1.Text == "" && txtId2.Text == "" && dtDat1.Text == "" && dtDat2.Text == "")
                sql = @"SELECT * From development_calvin with(nolock)";
            else
            {                
                if (txtId1.Text != "")
                {
                ...
                if (txtTrim_code1.Text != "")
                {
                    sql = sql + string.Format(" and trim_code>='{0}'", txtTrim_code1.Text);
                }
                if (txtTrim_code2.Text != "")
                {
                    sql = sql + string.Format(" and trim_code<='{0}'", txtTrim_code2.Text);
                }
                if (dtDat1.Text != "")
                {
                    sql = sql + string.Format(" and form_date1>='{0}'", clsApp.Return_String_Date(dtDat1.Text));
                }
                if (dtDat2.Text != "")
                {
                    sql = sql + string.Format(" and form_date1<='{0}'", clsApp.Return_String_Date(dtDat2.Text));
                }
            }
            dtDetail = clsPublicOfCF01.GetDataTable(sql);
        ...
        private void ReSet_Datagrid_Value()
        {
            ...
            {
                //取得當前新增或修改的行
                dtReSet = clsPublicOfCF01.GetDataTable(string.Format("Select serial_no,create_by,create_date,update_by,update_date From dbo.development_calvin with(nolock) WHERE serial_no='{0}'", strSerial_no));
                if (mState == "NEW")
                {
                    ...
                    row["form_date1"] = clsApp.Return_String_Date(txtform_date1.Text);
                    row["form_date2"] = txtform_date2.Text;
                    ...
                    row["minimum_sample"] = txtminimum_sample.Text;
                    row["minimum_bulk"] = txtminimum_bulk.Text;
                    //=================================================================
                    ...
                    row["ck_mens"] = chkck_mens.Checked ? true : false;
                    row["ck_womens"] = chkck_womens.Checked ? true : false;
                    row["ck_df"] = chkck_df.Checked ? true : false;
                    row["ck_formal"] = chkck_formal.Checked ? true : false;
                    row["ck_kids"] = chkck_kids.Checked ? true : false;
                    row["ck_accessories"] = chkck_accessories.Checked ? true : false;
                    row["ck_jeans"] = chkck_jeans.Checked ? true : false;
                    row["ck_swim"] = chkck_swim.Checked ? true : false;
                    row["ck_underwear"] = chkck_underwear.Checked ? true : false;
                    row["mk"] = chkmk.Checked ? true : false;
                    row["ck_sportswear"] = chkck_sportswear.Checked ? true : false;
                ...
                {                                      
                    ...
                    dtDetail.Rows[row_reset]["form_date1"] = clsApp.Return_String_Date(txtform_date1.Text);
                    dtDetail.Rows[row_reset]["form_date2"] = txtform_date2.Text;
                    ...
                    dtDetail.Rows[row_reset]["minimum_sample"] = txtminimum_sample.Text;
                    dtDetail.Rows[row_reset]["minimum_bulk"] = txtminimum_bulk.Text;
                    //=================================================================
                    ...
                    dtDetail.Rows[row_reset]["ck_mens"] = chkck_mens.Checked ? true : false;
                    dtDetail.Rows[row_reset]["ck_df"] = chkck_df.Checked ? true : false;
                    dtDetail.Rows[row_reset]["ck_formal"] = chkck_formal.Checked ? true : false;
                    dtDetail.Rows[row_reset]["ck_womens"] = chkck_womens.Checked ? true : false;                    
                    dtDetail.Rows[row_reset]["ck_kids"] = chkck_kids.Checked ? true : false;
                    dtDetail.Rows[row_reset]["ck_accessories"] = chkck_accessories.Checked ? true : false;
                    dtDetail.Rows[row_reset]["ck_jeans"] = chkck_jeans.Checked ? true : false;
                    dtDetail.Rows[row_reset]["ck_swim"] = chkck_swim.Checked ? true : false;                  
                    dtDetail.Rows[row_reset]["ck_underwear"] = chkck_underwear.Checked ? true : false;
                    dtDetail.Rows[row_reset]["mk"] = chkmk.Checked ? true : false;
                    dtDetail.Rows[row_reset]["ck_sportswear"] = chkck_sportswear.Checked ? true : false;
        ...
        private void Set_head(DataGridViewRow pdr)
        {
            ...
            txtform_date1.EditValue = Convert.ToDateTime(pdr.Cells["form_date1"].Value.ToString()).ToString("yyyy-MM-dd");
            txtform_date2.Text = pdr.Cells["form_date2"].Value.ToString();
            ...
            txtminimum_sample.Text = pdr.Cells["minimum_sample"].Value.ToString();
            txtminimum_bulk.Text = pdr.Cells["minimum_bulk"].Value.ToString();         
            //=================================================================
            ...
            chkck_mens.Checked = pdr.Cells["ck_mens"].Value.ToString() == "True" ? true : false;
            chkck_womens.Checked = pdr.Cells["ck_womens"].Value.ToString() == "True" ? true : false;
            chkck_df.Checked = pdr.Cells["ck_df"].Value.ToString() == "True" ? true : false;
            chkck_formal.Checked = pdr.Cells["ck_formal"].Value.ToString() == "True" ? true : false;
            chkck_kids.Checked = pdr.Cells["ck_kids"].Value.ToString() == "True" ? true : false;
            chkck_accessories.Checked = pdr.Cells["ck_accessories"].Value.ToString() == "True" ? true : false;
            chkck_jeans.Checked = pdr.Cells["ck_jeans"].Value.ToString() == "True" ? true : false;
            chkck_swim.Checked = pdr.Cells["ck_swim"].Value.ToString() == "True" ? true : false;
            chkck_underwear.Checked = pdr.Cells["ck_underwear"].Value.ToString() == "True" ? true : false;
            chkmk.Checked = pdr.Cells["mk"].Value.ToString() == "True" ? true : false;
            chkck_sportswear.Checked = pdr.Cells["ck_sportswear"].Value.ToString() == "True" ? true : false;
        ...
        private void txtTrim_code1_Leave(object sender, EventArgs e)
        {
            txtTrim_code2.Text = txtTrim_code1.Text;
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            string strsql = string.Format(@"Select * FROM dbo.development_calvin WHERE serial_no='{0}'",txtserial_no.Text);
            DataTable dtReport = clsPublicOfCF01.GetDataTable(strsql);
            if (radGrp1.SelectedIndex == 0)
            {
                using (xrDevelopCalvinSheet1 rpt = new xrDevelopCalvinSheet1() { DataSource = dtReport })
                {
                    rpt.CreateDocument();
            ...
            {
                using (xrDevelopCalvinSheet2 rpt = new xrDevelopCalvinSheet2() { DataSource = dtReport })
                {
                    rpt.CreateDocument();
                    rpt.PrintingSystem.ShowMarginsWarning = false;
                    rpt.ShowPreviewDialog();
        ...
        private void BTNNEWCOPY_Click(object sender, EventArgs e)
        {
            if (dgvDetails.RowCount > 0)
            {
                ...
                AddNew();
                Set_head(dgvrow);
                txtform_date1.EditValue = DateTime.Now.Date.ToString("yyyy/MM/dd").Substring(0, 10);
                string max_serial_no = clsTommyTest.GetSeqNo("development_calvin", "serial_no");
                ...
                using (frmTrimsSheetFind ofrmFind = new frmTrimsSheetFind(txttrim_code.Text))
                {
                    ...
                    if (ofrmFind.mList.Count > 0)
                    {
                        foreach (mdlQuotation stu in ofrmFind.mList)
                        {
                            ...
                            txtseason.EditValue = !string.IsNullOrEmpty(stu.season) ? stu.season : stu.season_desc;
                            txtmaterial.Text = stu.material;
                            txtsize.Text = stu.size;
                            txtcolor.Text = stu.cf_color;
                            lkem_id1.EditValue = "USD";
                            txtprice1.Text = stu.price_usd.ToString();
                            lkep_unit1.EditValue = stu.price_unit;
                        }
