...
namespace cf01.Forms
{
    public partial class frmShowPlan : Form
    {
        ...
        {
            DataTable dtPlanMostly = clsPlan.GetProductionPlanMostly(pMo_id);
            if (dtPlanMostly.Rows.Count > 0)
            {
                ...
                txtOrder_qty.Text = dtPlanMostly.Rows[0]["order_qty"].ToString();
        ...
        private void GenerateDataForPrint()
        {
            ...
            DataTable dtTransfer = clsPrdTransfer.GetTransferInfoByShowPlan(Mo_id);
            for (int i = 0; i < dtTransfer.Rows.Count; i++)
            {
                ...
                bool noPrintQc = false;
                DataTable dtTempData = clsPrdTransfer.GetPOData(Mo_id, wp_id, mater_id,noPrintQc);
            ...
            if (dtDataForPrint.Rows.Count > 0)
            {
                //配件名稱及顏色
                string id, seq;
                id = dtDataForPrint.Rows[0][""].ToString();
                seq = dtDataForPrint.Rows[0]["sequence_id"].ToString();
