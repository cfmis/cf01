...
namespace cf01.Forms
{
    public partial class frmGroup : Form
    {
        ...
        private bool Valid_Doc() //主建是否已存在
        {
            ...
            string strSql = String.Format("Select '1' FROM bs_group WHERE grp_code='{0}'", doc);
        ...
        private void Find_doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            ...
            {
                string strsql = String.Format("SELECT grp_code as id,grp_cdesc as cdesc,grp_desc as edesc,grp_rmk as rmk,crusr,crtim,amusr,amtim FROM dbo.bs_group WHERE grp_code='{0}'", temp_id);
                DataTable dt = clsPublicOfCF01.GetDataTable(strsql);
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (mState == "NEW")
            {
                ...
                const string sql_new = "INSERT INTO bs_group(grp_code,grp_desc,grp_cdesc,grp_rmk,crusr,crtim) values(@id,@edesc,@cdesc,@remark,@crusr,GETDATE())";
            ...
            if (mState == "EDIT")
            {
                const string sql_update = "UPDATE bs_group SET grp_cdesc=@cdesc,grp_desc=@edesc,grp_rmk=@remark,amusr=@amusr,amtim=GETDATE() WHERE grp_code=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        private void Delete() //刪除當前行
        {
            ...
            {
                const string sql_del = "DELETE FROM bs_group WHERE grp_code=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        {
            dgvDetails.Focus();
            DataTable dt = clsPublicOfCF01.GetDataTable("SELECT grp_code as id ,grp_cdesc as cdesc,grp_desc as edesc,grp_rmk as rmk,crusr,crtim,amusr,amtim FROM dbo.bs_group");
            dgvDetails.DataSource = dt;
        }
