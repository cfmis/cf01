...
namespace cf01.Forms
{
    public partial class frmZone : Form
    {
        ...
        private bool Valid_Doc() //主建是否已存在
        {
            ...
            string strSql = String.Format("Select '1' FROM bs_zone WHERE id='{0}'", doc);
        ...
        private void Find_doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            ...
            {
                string strsql = String.Format("SELECT id,cdesc,edesc,crusr,crtim,amusr,amtim FROM dbo.bs_zone WHERE id='{0}'", temp_id);
                DataTable dt = clsPublicOfCF01.GetDataTable(strsql);
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (mState == "NEW")
            {
                ...
                const string sql_new = "INSERT INTO bs_zone(id,edesc,cdesc,crusr,crtim) values(@id,@edesc,@cdesc,@crusr,getdate())";
            ...
            if (mState == "EDIT")
            {
                const string sql_update = "UPDATE bs_zone SET edesc=@edesc,cdesc=@cdesc,amusr=@amusr,amtim=getdate() WHERE id=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        private void Delete() //刪除當前行
        {
            ...
            {
                const string sql_del = "DELETE FROM bs_zone WHERE id=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        {
            dgvDetails.Focus();
            DataTable dt = clsPublicOfCF01.GetDataTable("SELECT id ,cdesc,edesc,crusr,crtim,amusr,amtim FROM dbo.bs_zone");
            dgvDetails.DataSource = dt;
        }
