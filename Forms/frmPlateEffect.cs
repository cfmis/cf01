...
{
    public partial class frmPlateEffect : Form
    {
        ...
        private bool Valid_Doc() //主建是否已存在
        {
            ...
            string strSql = String.Format("Select '1' FROM bs_plate_effect WHERE ple_code='{0}'", doc);
        ...
        private void Find_doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            ...
            {
                string strsql = String.Format("SELECT ple_code as id,ple_cdesc as cdesc,ple_desc as edesc,ple_rmk as rmk,crusr,crtim,amusr,amtim FROM dbo.bs_plate_effect WHERE ple_code='{0}'", temp_id);
                DataTable dt = clsPublicOfCF01.GetDataTable(strsql);
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (mState == "NEW")
            {
                ...
                const string sql_new = "INSERT INTO bs_plate_effect(ple_code,ple_desc,ple_cdesc,ple_rmk,crusr,crtim) values(@id,@edesc,@cdesc,@remark,@crusr,GETDATE())";
            ...
            if (mState == "EDIT")
            {
                const string sql_update = "UPDATE bs_plate_effect SET ple_cdesc=@cdesc,ple_desc=@edesc,ple_rmk=@remark,amusr=@amusr,amtim=GETDATE() WHERE ple_code=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        private void Delete() //刪除當前行
        {
            ...
            {
                const string sql_del = "DELETE FROM bs_plate_effect WHERE ple_code=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        {
            dgvDetails.Focus();
            DataTable dt = clsPublicOfCF01.GetDataTable("SELECT ple_code as id ,ple_cdesc as cdesc,ple_desc as edesc,ple_rmk as rmk,crusr,crtim,amusr,amtim FROM dbo.bs_plate_effect");
            dgvDetails.DataSource = dt;
        }
