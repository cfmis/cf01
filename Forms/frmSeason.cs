...
namespace cf01.Forms
{
    public partial class frmSeason : Form
    {
        ...
        private void Find_doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            ...
            {
                string strsql = String.Format("SELECT id,cdesc,edesc,remark as rmk,crusr,crtim,amusr,amtim FROM dbo.dbo.bs_season WHERE id='{0}'", temp_id);
                DataTable dt = clsPublicOfCF01.GetDataTable(strsql);                
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (mState == "NEW")
            {
                ...
                const string sql_new = "INSERT INTO dbo.bs_season(id,edesc,cdesc,remark,crusr,crtim) values(@id,@edesc,@cdesc,@remark,@crusr,getdate())";
            ...
            if (mState == "EDIT")
            {
                const string sql_update = "UPDATE dbo.bs_season SET edesc=@edesc,cdesc=@cdesc,remark=@remark,amusr=@amusr,amtim=getdate() WHERE id=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        {
            dgvDetails.Focus();
            DataTable dt = clsPublicOfCF01.GetDataTable("SELECT id ,cdesc,edesc,remark as rmk,crusr,crtim,amusr,amtim FROM dbo.bs_season");
            dgvDetails.DataSource = dt;
        }
