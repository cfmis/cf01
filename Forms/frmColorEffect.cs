...
{
    public partial class frmColorEffect : Form
    {
        ...
        private bool Valid_Doc() //主建是否已存在
        {
            ...
            string strSql = String.Format("Select '1' FROM bs_color_effect WHERE clr_code='{0}'", doc);
        ...
        private void Find_doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            ...
            {
                string strsql = String.Format("SELECT clr_code as id,clr_cdesc as cdesc,clr_desc as edesc,clr_rmk as rmk,crusr,crtim,amusr,amtim FROM dbo.bs_color_effect WHERE clr_code='{0}'", temp_id);
                DataTable dt = clsPublicOfCF01.GetDataTable(strsql);
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (mState == "NEW")
            {
                ...
                const string sql_new = "INSERT INTO bs_color_effect(clr_code,clr_desc,clr_cdesc,clr_rmk,crusr,crtim) values(@id,@edesc,@cdesc,@remark,@crusr,GETDATE())";
            ...
            if (mState == "EDIT")
            {
                const string sql_update = "UPDATE bs_color_effect SET clr_cdesc=@cdesc,clr_desc=@edesc,clr_rmk=@remark,amusr=@amusr,amtim=GETDATE() WHERE clr_code=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        private void Delete() //刪除當前行
        {
            ...
            {
                const string sql_del = "DELETE FROM bs_color_effect WHERE clr_code=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.connectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        {
            dgvDetails.Focus();
            DataTable dt = clsPublicOfCF01.GetDataTable("SELECT clr_code as id ,clr_cdesc as cdesc,clr_desc as edesc,clr_rmk as rmk,crusr,crtim,amusr,amtim FROM dbo.bs_color_effect");
            dgvDetails.DataSource = dt;
        }
