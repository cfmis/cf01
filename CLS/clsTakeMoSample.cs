...
namespace cf01.CLS
{
    public class clsTakeMoSample
    {
        ...
        //從生產計劃中獲取倉庫發料的記錄
        public DataTable GetFlRecFromJo(string id,string seq)
        {
            DataTable dtFlRec = new DataTable();
            string strSql = "";
        ...
        public int GetRate(string goods_id)
        {
            int result = 0;
            ...
            DataTable dtRate = clsConErp.ExecuteSqlReturnDataTable(strSql);
            if (dtRate.Rows.Count > 0)
                result = (dtRate.Rows[0]["rate"].ToString() != "" ? Convert.ToInt32(dtRate.Rows[0]["rate"]) : 0);
            return result;
        }
        //產生新的發料單號
        public string AddNewDoc(takemosample objModel)
        {
            int result = 0;
            string strSql = "";
            string strSql_f = "";
        ...
        //檢查單據是否有不同的倉庫號
        public DataTable checkDocDiffLoc(string id,string adn_id,string seq)
        {
            DataTable dtId = new DataTable();
        ...
        //提取單據明細表
        public DataTable loadDocSubRec(string id,string seq)
        {
            DataTable dtId = new DataTable();
        ...
        //查找MO庫存
        public DataTable checkMoStore(string loc,string mo_id,string goods_id,string lot_no)
        {
            ...
            dtMoStore = clsConErp.ExecuteSqlReturnDataTable(strSql);
            return dtMoStore;
        }
        //插入發貨記錄到明細表
        public string AddStoreRec(List<takemosample> listDetail)
        {
            int result = 0;
            string strSql = "";
            string unit = "PCS";
        ...
        private string GetSub_Seq_id(string sub_seq)
        {
            string sub_seq_id = "";
        ...
        //查找制單發貨資料
        public DataTable findDocRec(string id,string loc,string mo_id,string goods_id)
        {
            DataTable dtId = new DataTable();
        ...
        //查找制單發貨資料
        public DataTable findDocRec_Sub(string id, string loc, string goods_id, string state)
        {
            DataTable dtId = new DataTable();
            string strSql = "";
        ...
        //查找單據是否已批準
        public DataTable findDocDetails(string loc, string dat,string state,string tdep,string crusr)
        {
            DataTable dtId = new DataTable();
            string status="0";
            if (state == "0")
        ...
        //刪除發貨記錄
        public int DelStoreRec(string id,string seq)
        {
            int result = 0;
            string strSql = "";
        ...
        //查找庫存記錄
        public DataTable findStore(string loc,string mo_id, string goods_id)
        {
            string strSql = "";
            ...
            dtStore = clsConErp.ExecuteSqlReturnDataTable(strSql);
            return dtStore;
        }

        //從生產計劃中提取是否有QC流程
        public DataTable GetWipQc(string mo_id, string seq)
        {
        ...
        //從生產計劃中獲取倉庫發料的記錄QC
        public DataTable GetFlRecFromJoQc(string mo_id, string seq)
        {
            DataTable dtFlRec = new DataTable();
