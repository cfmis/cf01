...
namespace cf01.CLS
{
    public class clsCommonUse
    {
            ...
            try
            {
                ...
                switch (strType)
                {
                    case "ComboBox":
                        ...
                        cbx.BeginUpdate();
                        cbx.DataSource = clsPublicOfCF01.ExcuteSqlReturnDataSet(strSql, strTable).Tables[strTable];
                    ...
                    case "DataGridViewComboBoxColumn":
                        ...
                        dgvcbx.DataSource = clsPublicOfCF01.ExcuteSqlReturnDataSet(strSql, strTable).Tables[strTable];
        ...
        public void BuilDataGridView(Object obj, string form_name, string lang_id)
        {
            string colid, headername;
            int colwidth;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    //DataRow dr = rows2[i];
                    colid = dr["tb_col_name"].ToString().Trim();
                    headername = dr["show_name"].ToString().Trim();
                    colwidth = (Int32)dr["tb_col_width"];
                    dgv.Columns.Add(colid, headername);
                    dgv.Columns[colid].DataPropertyName = colid;
                    dgv.Columns[colid].Width = colwidth;
                }
            }
